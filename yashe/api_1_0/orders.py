...
from yashe import db, redis_store
from . import api
from yashe.models import House, Order
...
def save_order():
    ...
    start_date =order_data.get('start_date') #获取的是字符串形式
    ...
    try:
        #查询时间冲突的订单数
        counts =Order.query.filter(Order.house_id==house_id,Order.begin_date<=end_date,Order.end_date>=start_date).count()
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg='数据库订单查询失败')
    if counts >0:
...
def get_user_orders():
    '''查询用户订单'''
    user_id =g.user_id
    # 获取用户的身份，用户想要查询作为房客预订别人房子的订单，还是想要作为房东查询别人预订自己房子的订单
...
def save_order_comment(order_id):
    ...
    try:
        #需要确保只能评论自己下的订单，而且订单处于带评价状态才可以进行评价
